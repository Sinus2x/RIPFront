...
const productSlice = createSlice<ProductState, SliceCaseReducers<ProductState>>({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getProductListAction.fulfilled, (state, { payload }) => {
            state.status = FetchStatus.FULFILLED;
            const availableGenres: (string | undefined)[] = [];
            payload?.forEach(
                (product) => !availableGenres.includes(product.brand) && availableGenres.push(product.brand)
            );
            state.availableGenres = availableGenres;
